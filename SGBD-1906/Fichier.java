...
public class Fichier extends File {
    ...
    public Fichier( int port )throws Exception{
        super("port/port.txt");
        try{
            String fileResponse = this.getPortFile();
            String[] fileSplit = fileResponse.split("@");
            this.port = Integer.valueOf(fileSplit[0]);
            host = (fileSplit[1]);
        }catch( Exception e ){
            throw e;
        }
...
///setters
    ...
    public String getPortFile()throws Exception{
        BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(this), ("UTF-8")));
        ...
        if (port.matches("[+-]?\\d*(\\.\\d+)?") == false || port.length() != 4){
            throw new Exception(" port invalide ");
        }
        
        line = br.readLine();

        port = port +  "@" + (line.split(":")[1]).trim();
